      ...
      <div class="post-list">
        ...
        <div class="load-more" v-if="hasMore">
          <el-button type="text" @click="loadMore">
            더 보기 <i class="el-icon-arrow-down"></i>
          </el-button>
        </div>
      </div>
          ...
          <div class="ranking-list">
            <div v-for="(post, index) in commentRanking" 
              ...
              <div class="ranking-content">
                ...
                <div class="ranking-count">{{ post.num2 }} 댓글</div>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    toggleLike(post) {
      if(this.type!=='02'){
            this.$message.warning("일반 사용자만 즐겨찾기 할 수 있습니다.");
            return;
        }
        if(this.userInfo!=null){
            ...
            this.$axios.post('/api/stars/add', param).then(res => {
                if(res.data.code == '200'){
                    this.$message.success("즐겨찾기에 추가되었습니다.");
                    this.getData();
                }else{
                    this.$message.warning("즐겨찾기 실패");
                }
            })
        }else{
            this.$message.warning("로그인 후 이용해주세요.");
        }
    },
    viewPostDetail(id) {
    ...
    sharePost(post) {
      this.$message.success('공유 기능 준비 중입니다.')
    },
    loadMore() {
      if(this.hasMore) {
        this.currentPage++;
    ...
    getCommentRanking() {
      ...
      this.$axios.post('/api/forum/frontPage2', param).then(res => {
        if(res.data.code == 200) {
