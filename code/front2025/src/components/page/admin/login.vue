  ...
  <div class="login-wrap">
    <div class="ms-login">
      <div class="login-header">
        <h2>패션 스타일</h2>
        <p>패션 커뮤니티에 오신 것을 환영합니다</p>
      </div>

      <el-form :model="param" :rules="rules" ref="login" label-width="0px" class="ms-content">
        <el-form-item prop="phone">
          <el-input v-model="param.phone" placeholder="계정을 입력하세요">
        ...
        <el-form-item prop="type">
          <el-select v-model="param.type" placeholder="로그인 유형 선택" style="width: 100%">
            <el-option label="사용자" value="02"></el-option>
            <el-option label="관리자" value="01"></el-option>
          </el-select>
        </el-form-item>
        <div class="login-btn">
          <el-button type="primary" round @click="submitForm()">로그인</el-button>
        </div>
      </el-form>

      <div class="login-footer">
        <router-link to="/" class="footer-link">홈으로 돌아가기</router-link>
        <router-link to="/userregister" class="footer-link">지금 가입하기</router-link>
      </div>
    </div>
  </div>
</template>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    phone: [{required: true, message: '계정을 입력하세요', trigger: 'blur'}],
                    password: [{required: true, message: '비밀번호를 입력하세요', trigger: 'blur'}],
                    type: [{required: true, message: '사용자 유형을 선택하세요', trigger: 'blur'}],
                },
            }
        },
