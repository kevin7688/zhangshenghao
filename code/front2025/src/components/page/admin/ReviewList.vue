  ...
  <div>
    <div class="container">
      <div class="handle-box">
        <el-input v-model="query.goodsName" placeholder="상품 이름을 입력하세요" class="handle-input mr10"></el-input>
         <el-button type="primary" icon="el-icon-search" @click="getData">검색</el-button>
      </div>
      <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header" >
        <el-table-column type="selection" width="55" align="center"></el-table-column>
        <el-table-column prop="goodsName" label="이름" align="center">
    ...
    <el-dialog :title="dialogName" :visible.sync="dialogVisible" width="35%">
        ...
        <el-form-item label="댓글 내용" prop="content">
            <el-input v-model="form.content" type="textarea" rows="3" placeholder="댓글 내용을 입력하세요"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getData() {
        ...
        this.$axios.post('/api/review/selectPage',param).then(res => {
    ...
    handleAdd() {
        ...
        this.form = {};
        this.$refs['ruleForm'].clearValidate();
        this.dialogName = "상품 댓글 추가";
    },
    // 저장
    save(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post(this.form.id?'/api/review/edit' : '/api/review/add', this.form).then(res => {
              if(res.data.code == 200){
    ...
    handleDelete(index, row) {
        this.$confirm('정말 삭제하시겠습니까?', '알림', {
           type: 'warning'
             ...
             this.$axios.get('/api/review/deleteById?id='+row.id).then(res => {
                if(res.data.code == 200){
                    this.$message.success(res.data.msg);
