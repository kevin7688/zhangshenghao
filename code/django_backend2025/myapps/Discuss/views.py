...
from myapps.Discuss.models import Discuss, DiscussSerializer
...
class selectOneView(View):
    def get(self, request):
        try:
            ...
            data = Discuss.objects.filter(id=id).first()
            if not data:
                return ResponseHandler.error(msg='数据不存在')
            data_serializer = DiscussSerializer(instance=data)
...
class frontAllView(BaseQueryView):
    queryset = Discuss.objects.order_by('-create_time')
    serializer_class = DiscussSerializer
    def post(self, request):
        try:
            # 构建查询条件
...
class addView(View):
    def post(self, request):
        try:
            ...
            serializer_obj = DiscussSerializer(data=dic)
            ...
            return ResponseHandler.success(
                data=DiscussSerializer(insert_data).data,
                msg='添加成功'
            )
        except Exception as e:
            print("添加出现异常: %s" % e)
...
class editView(View):
    def post(self, request):
        try:
            ...
            obj = Discuss.objects.filter(id=id_value).first()
            if not obj:
                return ResponseHandler.error(msg='数据不存在')
            #使用 ModelHelper 更新字段
            ModelHelper.update_model_fields(
            ...
            obj.save()  # 保存更新
            return ResponseHandler.success(data=DiscussSerializer(obj).data, msg='更新成功')
        except Exception as e:
            print("更新时出现异常: %s" % e)
...
class DelView(View):
    def get(self, request):
        try:
            id = request.GET.get("id")
            discuss = Discuss.objects.filter(id=id).first()
            if not discuss:
                return ResponseHandler.error(msg='数据不存在')
            discuss.delete()
    ...
    try:
        ...
        if uid2:
            queryset = queryset.filter(
                Q(uid_id=uid2) | Q(oid__uid_id=uid2)
            )
