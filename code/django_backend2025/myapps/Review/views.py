...
class selectPageView(generics.GenericAPIView):
    queryset = Review.objects.order_by('-create_time')
    ...
    def post(self, request):
        try:
            ...
            if item_page:
                item_serializer = ReviewSerializer(instance=item_page, many=True)
                return ResponseHandler.success_with_page(
                    list_data=item_serializer.data,
                    total=total,
                    msg='获取成功'
                )
            serializer = ReviewSerializer(instance=item_queryset, many=True)
            return ResponseHandler.success_with_page(
...
class selectOneView(View):
    def get(self, request):
        try:
            ...
            data = Review.objects.filter(id=id).first()
            if not data:
                return ResponseHandler.error(msg='数据不存在')
            data_serializer = ReviewSerializer(instance=data)
...
class addView(View):
    def post(self, request):
        try:
            ...
            serializer_obj = ReviewSerializer(data=dic)
            ...
            return ResponseHandler.success(
                data=ReviewSerializer(insert_data).data,
                msg='添加成功'
            )
        except Exception as e:
            print("添加出现异常: %s" % e)
...
class editView(View):
    def post(self, request):
        try:
            ...
            obj = Review.objects.filter(id=id_value).first()
            if not obj:
                return ResponseHandler.error(msg='数据不存在')
            #使用 ModelHelper 更新字段
            ModelHelper.update_model_fields(
            ...
            obj.save()  # 保存更新
            return ResponseHandler.success(data=ReviewSerializer(obj).data, msg='更新成功')
        except Exception as e:
            print("更新时出现异常: %s" % e)
