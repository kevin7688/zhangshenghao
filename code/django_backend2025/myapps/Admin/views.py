...
class selectPageView(generics.GenericAPIView):
    queryset = Admin.objects.order_by('-create_time')
    ...
    def post(self, request):
        try:
            ...
            if item_page:
                item_serializer = AdminSerializer(instance=item_page, many=True)
                return ResponseHandler.success_with_page(
                    list_data=item_serializer.data,
                    total=total,
                    msg='获取成功'
                )
            serializer = AdminSerializer(instance=item_queryset, many=True)
            return ResponseHandler.success_with_page(
...
class selectOneView(View):
    def get(self, request):
        try:
            ...
            data_serializer = AdminSerializer(instance=data)
...
class addView(View):
    def post(self, request):
        try:
            ...
            serializer_obj = AdminSerializer(data=dic)
            if not serializer_obj.is_valid():
                return ResponseHandler.error(msg=serializer_obj.errors)
            Admin.objects.create(**serializer_obj.validated_data)
...
class editView(View):
    def post(self, request):
        try:
            ...
            obj = Admin.objects.filter(id=id_value).first()
            if not obj:
                return ResponseHandler.error(msg='数据不存在')
            #使用 ModelHelper 更新字段
            ...
            obj.save()  # 保存更新
            return ResponseHandler.success(data=AdminSerializer(obj).data, msg='更新成功')
        except Exception as e:
            print("更新时出现异常: %s" % e)
